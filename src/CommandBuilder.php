...
class CommandBuilder implements interfaces\CommandBuilder
{
     ...
     *
     * @return interfaces\commands\Command
     */
    public function build(string $className): interfaces\commands\Command
    {
        $params = is_subclass_of($className, interfaces\commands\ExecutableCommand::class)
            ? [$this->bus]
            : [];

        /** @var interfaces\commands\Command $command */
        $command = new $className(...$params);
     ...
     */
    protected function prepareCommand(interfaces\commands\Command $command): void
    {
